...
class Application(ttk.Frame):
    ...
    def create_widgets_initial(self):
        ...
        self.select_file_button = ttk.Button(self.left_frame, text="Select file", command=self.select_file)
    ...
    def create_widgets_after_file_load(self):
        ...
        self.country_code_label = ttk.Label(self.column_selection_frame, text="Select ISO Code Column:")
        ...
        self.var_label.grid(row=1, column=0, padx=5, pady=5, sticky="w")
        ...
        self.var_menu.grid(row=1, column=1, padx=5, pady=5, sticky="ew")

        self.color_scheme_label = ttk.Label(self.column_selection_frame, text="Select Color Scheme:")
        self.color_scheme_label.grid(row=2, column=0, padx=5, pady=5, sticky="w")

        self.color_scheme_var = tk.StringVar(self)
        self.color_scheme_var.set(self.color_scheme)
        # Create the color scheme dropdown
        self.color_scheme_menu = ttk.OptionMenu(self.column_selection_frame, self.color_scheme_var, 'viridis', 'viridis', 'plasma', 'inferno', 'magma', 'cividis')
        self.color_scheme_menu.grid(row=2, column=1, padx=5, pady=5, sticky="ew")

        self.lat_label = ttk.Label(self.column_selection_frame, text="Select Latitude Column:")
        self.lat_label.grid(row=3, column=0, padx=5, pady=5, sticky="w")
        ...
        self.lat_menu.grid(row=3, column=1, padx=5, pady=5, sticky="ew")

        self.lon_label = ttk.Label(self.column_selection_frame, text="Select Longitude Column:")
        self.lon_label.grid(row=4, column=0, padx=5, pady=5, sticky="w")
        ...
        self.lon_menu.grid(row=4, column=1, padx=5, pady=5, sticky="ew")
    ...
    def plot(self):
        ...
        if plot_type == 'choropleth':
            ...
            merged_data = self.geo_data.set_index('ISO_A3').join(self.data.set_index(self.country_code_var.get()))  # Assuming the first column has the data
            ...
            for index, row in self.data.iterrows():
                lat = row[self.lat_var.get()]
                lon = row[self.lon_var.get()]
                ax.plot(lon, lat, 'o', color=self.dot_color, markersize=self.dot_size_slider.get(), transform=ccrs.PlateCarree())
    ...
    def save_to_disk(self):
        ...
        if file_path:
            ...
            if plot_type == 'choropleth':
                ...
                scaled_dot_size = self.dot_size_slider.get() * scale_factor
                ...
                for index, row in self.data.iterrows():
                    lat = row[self.lat_var.get()]
                    lon = row[self.lon_var.get()]
                    high_res_ax.plot(lon, lat, 'o', color=self.dot_color, markersize=scaled_dot_size, transform=ccrs.PlateCarree())
    ...
    def reset(self):
        ...
        for widget in self.left_frame.winfo_children():
            if widget not in {self.select_file_button, self.header_check, self.plot_type_frame, self.load_file_button, self.reset_plot_frame}:
                widget.destroy()
        for widget in self.map_frame.winfo_children():
            widget.destroy()
